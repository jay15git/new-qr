...
import { remapOpacityToTriplet, SOURCE_BASE_OPACITY, SOURCE_PEAK_OPACITY, } from './opacity-triplet';
...
(function (AnimationPreset) {
    ...
    AnimationPreset["GlyphPulse"] = "GlyphPulse";
    AnimationPreset["CRTGlide"] = "CRTGlide";
    AnimationPreset["EchoRing"] = "EchoRing";
    AnimationPreset["OriginWave"] = "OriginWave";
    AnimationPreset["CoreRotor"] = "CoreRotor";
...
const SoftMaterialize = (targets, x, y, count, entity) => {
    const center = count / 2;
...
const SubtlePulse = (targets, _x, _y, _count, entity) => ({
    targets,
...
const FinderPing = (targets, _x, _y, _count, entity) => ({
    targets,
    from: entity === QRCodeEntity.Module ? 120 : 0,
    duration: entity === QRCodeEntity.PositionRing ||
        entity === QRCodeEntity.PositionCenter ||
        entity === QRCodeEntity.Icon
        ? 850
        : 360,
    easing: 'cubic-bezier(0.215, 0.610, 0.355, 1.000)',
    web: entity === QRCodeEntity.PositionRing ||
        entity === QRCodeEntity.PositionCenter ||
        entity === QRCodeEntity.Icon
        ? {
            opacity: [1, 0.72, 1],
...
const CenterBloom = (targets, x, y, count, entity) => {
    const { adjustedX, adjustedY } = innermostPoint(x, y, count, entity);
    const center = count / 2;
    ...
    return {
        ...
        web: {
            opacity: [0, 1],
            scale: entity === QRCodeEntity.Icon ? [0.82, 1.08, 1] : [0.2, 1.15, 0.96, 1],
        },
    };
};
const CornerSweep = (targets, x, y, count, entity) => {
    const distances = [
...
const SignalScan = (targets, x, y, count, entity) => {
    const center = count / 2;
...
const coordinateSeed = (x, y, count) => Math.abs(Math.round((x + 1) * 37 + (y + 1) * 61 + count * 17));
...
const matrixFracCoord = (x, y, count) => {
...
const discreteCellField = (fRow, fCol, fn) => {
...
const sampleCellField = (fRow, fCol, fn) => {
...
const matrixCssKeyframe = (offset, peak, mid, base) => ({
    cssBlend: { base, mid, peak },
    offset,
});
const keyframeNumericValue = (frame) => typeof frame === 'number' ? frame : frame.value;
const keyframeOpacityAt = (frames, phase) => {
    const clampedPhase = clamp(phase, 0, 1);
    if (frames.length === 0)
        return SOURCE_BASE_OPACITY;
    if (frames.length === 1) {
...
const rowMajorIndex = (row, col) => row * MATRIX_SIZE + col;
const matrixManhattanDistance = (row, col) => Math.abs(row - 2) + Math.abs(col - 2);
const matrixSourceStyle = (targets, _entity, from, duration, opacity, easing = 'linear') => ({
    targets,
    from,
    duration,
    easing,
...
const matrixEntityAnimation = (targets, entity, duration = 520) => matrixSourceStyle(targets, entity, 0, duration, [1, 0.86, 1], 'ease-in-out');
const steppedOpacityFrames = (steps, opacityForStep) => {
...
const phaseOpacityFrames = (samples, opacityForPhase) => steppedOpacityFrames(samples + 1, (step) => opacityForPhase(step / Math.max(1, samples)));
const trBlPathNormFromCoord = (row, col) => (row + (MATRIX_LAST - col)) / (MATRIX_LAST * 2);
const buildSpiralInwardOrderToIndexMap = () => {
    const order = new Array(MATRIX_CELLS);
    let top = 0;
...
const spiralInwardOrderValue = (index) => SPIRAL_INWARD_ORDER[index];
...
const outerRingClockwiseOrderValue = (index) => OUTER_RING_CLOCKWISE_ORDER[index];
const middleRingAntiClockwiseOrderValue = (index) => MIDDLE_RING_ANTI_CLOCKWISE_ORDER[index];
const ConfettiPop = (targets, x, y, count, entity) => {
    ...
    return {
        ...
        easing: 'cubic-bezier(0.175, 0.885, 0.320, 1.275)',
        web: {
            opacity: entity === QRCodeEntity.Module ? [0, 1, 0.82, 1] : [1, 0.72, 1],
            scale: entity === QRCodeEntity.Module
                ? [0.08, 1.36, 0.88, 1]
                : [0.86, 1.18, 0.97, 1],
        },
...
const SpiralBloom = (targets, x, y, count, entity) => {
    const { adjustedX, adjustedY } = innermostPoint(x, y, count, entity);
    const center = count / 2;
...
const BubbleCascade = (targets, x, y, count, entity) => {
    const center = count / 2;
...
const KaleidoPulse = (targets, x, y, count, entity) => {
    const center = count / 2;
...
const FireflyTwinkle = (targets, x, y, count, entity) => {
    const seed = coordinateSeed(x, y, count);
...
const MagneticRipple = (targets, x, y, count, entity) => {
    const { adjustedX, adjustedY } = innermostPoint(x, y, count, entity);
    const leftPull = distanceBetween(adjustedX, adjustedY, count * 0.18, count * 0.72);
    const rightPull = distanceBetween(adjustedX, adjustedY, count * 0.82, count * 0.28);
...
const ParallaxTiles = (targets, x, y, count, entity) => {
    ...
    return {
        ...
        easing: 'cubic-bezier(0.175, 0.885, 0.320, 1.275)',
        web: {
            opacity: entity === QRCodeEntity.Module ? [0.18, 1, 0.84, 1] : [1, 0.74, 1],
            scale: entity === QRCodeEntity.Module
                ? tileDepth === 0
                    ? [0.42, 1.22, 0.95, 1]
                    : [1.28, 0.82, 1.1, 1]
...
const premiumGridBand = (value, count) => clamp(Math.floor((value / Math.max(1, count)) * PREMIUM_GRID_SIZE), 0, PREMIUM_GRID_LAST);
const premiumGridCoord = (x, y, count) => ({
    row: premiumGridBand(y, count),
    col: premiumGridBand(x, count),
});
const premiumDiagonalSnakeOrder = (row, col) => {
    let order = 0;
...
const ConstellationTrace = (targets, x, y, count, entity) => {
    const { row, col } = premiumGridCoord(x, y, count);
    const seed = coordinateSeed(x, y, count);
    const diagonalOrder = premiumDiagonalSnakeOrder(row, col);
    const elbowOrder = row <= 3
        ? row * PREMIUM_GRID_SIZE + Math.abs(col - 1) * 2
        : (PREMIUM_GRID_LAST - row) * PREMIUM_GRID_SIZE +
            Math.abs(col - 5) * 2 +
            18;
    const ladderOrder = col * PREMIUM_GRID_SIZE + (col % 2 === 0 ? row : PREMIUM_GRID_LAST - row);
    const routeOrder = seed % 3 === 0 ? diagonalOrder : seed % 3 === 1 ? elbowOrder : ladderOrder;
...
const ApertureReveal = (targets, x, y, count, entity) => {
    const { row, col } = premiumGridCoord(x, y, count);
    ...
    const bladeOffset = row % 2 === 0 ? verticalBlade * 30 : horizontalBlade * 30 + 42;
    ...
    return {
        ...
        easing: 'steps(6, end)',
        web: {
            opacity: entity === QRCodeEntity.Module ? [0, 0.72, 1, 0.82, 1] : [0.78, 1],
            scale: entity === QRCodeEntity.Module
...
const LensFocus = (targets, x, y, count, entity) => {
    const { row, col } = premiumGridCoord(x, y, count);
...
const ReceiptPrint = (targets, x, y, count, entity) => {
...
const FlipClock = (targets, x, y, count, entity) => {
...
const WaveInterference = (targets, x, y, _count, entity) => {
    const waveField = Math.sin(x * 0.9) + Math.sin(y * 0.9) + Math.sin((x + y) * 0.6);
...
const QuantumMaterialize = (targets, x, y, count, entity) => {
    const seed = coordinateSeed(x, y, count);
...
const MagneticSnap = (targets, x, y, count, entity) => {
    const noise = hashNoise(x, y, 73);
    const scatterX = (hashNoise(x, y, 11) - 0.5) *
        (entity === QRCodeEntity.Module ? 22 : 10);
    const scatterY = (hashNoise(x, y, 37) - 0.5) *
        (entity === QRCodeEntity.Module ? 22 : 10);
    const spin = (hashNoise(x, y, 53) - 0.5) *
...
const HoloFlicker = (targets, x, y, count, entity) => {
    const seed = coordinateSeed(x, y, count);
    const flickerDelay = (seed % 19) * 22;
    const hueShift = (seed % 5) * 18;
    return {
        targets,
        from: entity === QRCodeEntity.Module ? flickerDelay : flickerDelay * 0.3,
...
const ShockwaveJolt = (targets, x, y, _count, entity) => {
...
const TideRise = (targets, x, y, count, entity) => {
...
const GravityCollapse = (targets, x, y, count, entity) => {
    const seed = coordinateSeed(x, y, count);
    const angle = hashNoise(x, y, seed % 61) * Math.PI * 2;
    const distance = entity === QRCodeEntity.Module ? 18 + hashNoise(x, y, 7) * 14 : 10;
...
const frameMaskCell = (mask, row, col) => mask[rowMajorIndex(row, col)] || '.';
const NeonDrift = (targets, x, y, count, entity) => {
    ...
    return matrixSourceStyle(targets, entity, (path * 0.2 + parity * 0.5) * MATRIX_CYCLE_MS, MATRIX_CYCLE_MS, [
...
const PulseLadder = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1500, steppedOpacityFrames(SQUARE2_ROUTE.length, (step) => discreteCellField(fRow, fCol, (row, col) => opacityFromSquare2Tail(rowMajorIndex(row, col), step))), 'steps(33, end)');
};
const CoreSpiral = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const order = sampleCellField(fRow, fCol, (row, col) => spiralInwardOrderValue(rowMajorIndex(row, col)));
    return matrixSourceStyle(targets, entity, order * 0.04 * MATRIX_CYCLE_MS, MATRIX_CYCLE_MS, [
        matrixCssKeyframe(0, 0, 0, 0.5),
...
const twinOrbitDelayNorm = (row, col) => {
    if (row === 2 && col === 2)
        return -1;
...
const TwinOrbit = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const delayNorm = sampleCellField(fRow, fCol, twinOrbitDelayNorm);
    const quietKeyframes = [
        matrixCssKeyframe(0, 0, 0, 0.5),
        matrixCssKeyframe(1, 0, 0, 0.5),
    ];
    const ringKeyframes = [
    ...
    return matrixSourceStyle(targets, entity, Math.max(0, delayNorm) * MATRIX_CYCLE_MS, MATRIX_CYCLE_MS, delayNorm < 0 ? quietKeyframes : ringKeyframes);
};
const FluxColumns = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const position = sampleCellField(fRow, fCol, (row, col) => col % 2 === 0 ? MATRIX_LAST - row : row);
    return matrixSourceStyle(targets, entity, position * 0.2 * MATRIX_CYCLE_MS, MATRIX_CYCLE_MS, [
...
const blockDropCellOpacity = (row, col, step) => {
...
const BlockDrop = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1900, steppedOpacityFrames(SQUARE7_FRAME_SEQUENCE.length, (step) => discreteCellField(fRow, fCol, (row, col) => blockDropCellOpacity(row, col, step))), 'steps(11, end)');
};
const strobeStackCellOpacity = (row, col, step) => {
    ...
    if (step <= fillLast) {
        ...
        height = Math.max(0, Math.min(MATRIX_SIZE, MATRIX_SIZE - Math.max(0, drainTick - col)));
    }
...
const StrobeStack = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const fillLast = MATRIX_SIZE + MATRIX_SIZE - 1;
    ...
    return matrixSourceStyle(targets, entity, 0, 2000, steppedOpacityFrames(sequenceLen, (step) => discreteCellField(fRow, fCol, (row, col) => strobeStackCellOpacity(row, col, step))), 'steps(24, end)');
};
const square9KeyframesForBit = (bitIndex) => {
    const windows = [
    ...
    const frames = [{ offset: 0, value: SOURCE_BASE_OPACITY }];
...
const square9BitForCell = (row, col) => {
    if (row < 1 || row > 3)
        return null;
    const dr = row - 1;
    if (col === 0)
...
const square9StaticOpacityForCell = (row, col) => row >= 1 && row <= 3 && col === 2 ? 0.12 : 0.08;
const glyphPulseCellOpacity = (row, col, phase) => {
    const bit = square9BitForCell(row, col);
    if (bit === null)
        return square9StaticOpacityForCell(row, col);
    return keyframeOpacityAt(square9KeyframesForBit(bit), phase);
};
const GlyphPulse = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 5200, phaseOpacityFrames(52, (phase) => discreteCellField(fRow, fCol, (row, col) => glyphPulseCellOpacity(row, col, phase))), 'steps(52, end)');
};
const crtGlideCellOpacity = (row, col, scanRow) => {
    if (row > scanRow)
        return 0.08;
...
const CRTGlide = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1500, steppedOpacityFrames(MATRIX_SIZE, (scanRow) => crtGlideCellOpacity(fRow, fCol, scanRow)), 'steps(5, end)');
};
const EchoRing = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const ring = sampleCellField(fRow, fCol, (row, col) => clamp(matrixManhattanDistance(row, col), 0, 4));
    const parity = Math.round(ring) % 2;
    return matrixSourceStyle(targets, entity, (ring * 0.14 + parity * 0.03) * MATRIX_CYCLE_MS, MATRIX_CYCLE_MS, [
...
const OriginWave = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const ring = sampleCellField(fRow, fCol, (row, col) => clamp(Math.abs(row - 1) + Math.abs(col - 1), 0, 6));
    return matrixSourceStyle(targets, entity, ring * 0.16 * MATRIX_CYCLE_MS, MATRIX_CYCLE_MS, [
        matrixCssKeyframe(0, 0, 0, 0.625),
...
const prismBloomCellOpacity = (row, col, step) => {
...
const PrismBloom = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1700, steppedOpacityFrames(SQUARE14_FRAME_SEQUENCE.length, (step) => discreteCellField(fRow, fCol, (row, col) => prismBloomCellOpacity(row, col, step))), 'steps(6, end)');
};
const helixGlowCellOpacity = (row, col, phase) => {
...
const HelixGlow = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1600, phaseOpacityFrames(32, (phase) => sampleCellField(fRow, fCol, (row, col) => helixGlowCellOpacity(row, col, phase))));
};
const helixCoreCellOpacity = (row, col, phase) => {
    const rowPhase = phase * 20 * ((Math.PI * 2) / 19) + row * 1.24;
    const left = Math.round(1.5 + 0.5 * Math.sin(rowPhase));
    const right = 4 - left;
    if (col === left || col === right)
        return 1;
    if (col > left && col < right && Math.cos(rowPhase * 2) > 0.82)
        return 0.58;
    if (Math.abs(col - left) === 1 || Math.abs(col - right) === 1)
        return 0.24;
    return 0.08;
};
const HelixCore = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1400, phaseOpacityFrames(20, (phase) => sampleCellField(fRow, fCol, (row, col) => helixCoreCellOpacity(row, col, phase))));
};
const SQUARE19_STEP_COUNT = 48;
const square19GridPoint = (row, col) => ({
    x: (col - 2) / 2,
    y: (2 - row) / 2,
});
const square19LoopPoint = (step) => {
    const t = ((step % SQUARE19_STEP_COUNT) / SQUARE19_STEP_COUNT) * Math.PI * 2;
    return { x: Math.sin(t), y: 0.58 * Math.sin(2 * t) };
};
const square19DistanceSq = (a, b) => {
    const dx = a.x - b.x;
    const dy = a.y - b.y;
    return dx * dx + dy * dy;
};
const square19HeadInfluence = (dot, head) => Math.exp(-square19DistanceSq(dot, head) / 0.19);
const InfinityRun = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    const dot = square19GridPoint(fRow, fCol);
    return matrixSourceStyle(targets, entity, 0, 1700, steppedOpacityFrames(SQUARE19_STEP_COUNT, (step) => {
        ...
        const lead = Math.max(square19HeadInfluence(dot, headA), square19HeadInfluence(dot, headB));
        const trail = Math.max(square19HeadInfluence(dot, trailA), square19HeadInfluence(dot, trailB));
...
const square20OpacityFromTail = (distance, tail) => distance >= 0 && distance < tail.length ? tail[distance] : 0;
const mobiusRunCellOpacity = (row, col, step) => {
    const index = rowMajorIndex(row, col);
    let opacity = 0.08;
    const onLoop = SQUARE20_PERIMETER_PATH.indexOf(index);
    const backHead = (step + Math.floor(SQUARE20_PERIMETER_PATH.length / 2)) %
        SQUARE20_PERIMETER_PATH.length;
    if (onLoop >= 0) {
        const forward = (step - onLoop + SQUARE20_PERIMETER_PATH.length) %
            SQUARE20_PERIMETER_PATH.length;
        const alongBack = (backHead - onLoop + SQUARE20_PERIMETER_PATH.length) %
            SQUARE20_PERIMETER_PATH.length;
        opacity = Math.max(opacity, square20OpacityFromTail(forward, SQUARE20_TAIL_BRIGHT), square20OpacityFromTail(alongBack, SQUARE20_BACK_TAIL_BRIGHT));
    }
...
const MobiusRun = (targets, x, y, count, entity) => {
    if (entity !== QRCodeEntity.Module)
        return matrixEntityAnimation(targets, entity);
    const { fRow, fCol } = matrixFracCoord(x, y, count);
    return matrixSourceStyle(targets, entity, 0, 1600, steppedOpacityFrames(SQUARE20_PERIMETER_PATH.length, (step) => discreteCellField(fRow, fCol, (row, col) => mobiusRunCellOpacity(row, col, step))), 'steps(16, end)');
};
...
const dotMatrixColorSettings = (settings) => {
...
const safeAnimationSpeed = (settings) => {
    const speed = Number(settings && settings.animationSpeed);
    return speed > 0 && Number.isFinite(speed)
        ? speed
        : DEFAULT_ANIMATION_SPEED;
};
const dotMatrixOpacitySettings = (settings) => ({
    base: clamp(Number(settings && settings.dotMatrixOpacityBase !== undefined
        ? settings.dotMatrixOpacityBase
        : DEFAULT_DOT_MATRIX_OPACITY_BASE), 0, 1),
    mid: clamp(Number(settings && settings.dotMatrixOpacityMid !== undefined
        ? settings.dotMatrixOpacityMid
        : DEFAULT_DOT_MATRIX_OPACITY_MID), 0, 1),
    peak: clamp(Number(settings && settings.dotMatrixOpacityPeak !== undefined
        ? settings.dotMatrixOpacityPeak
        : DEFAULT_DOT_MATRIX_OPACITY_PEAK), 0, 1),
});
const isCssBlendKeyframe = (frame) => typeof frame === 'object' &&
    frame !== null &&
    'cssBlend' in frame &&
    typeof frame.cssBlend === 'object';
const resolveCssBlendOpacity = (blend, settings) => {
    const { base, mid, peak } = dotMatrixOpacitySettings(settings);
    return clamp(blend.peak * peak + blend.mid * mid + blend.base * base, 0, 1);
};
export const resolveDotMatrixKeyframeOpacity = (frame, settings) => {
    if (isCssBlendKeyframe(frame)) {
        return resolveCssBlendOpacity(frame.cssBlend, settings);
    }
    const raw = typeof frame === 'number'
        ? frame
...
const remapDotMatrixOpacityValue = (value, settings) => {
    if (!Number.isFinite(value))
        return value;
    const { base, mid, peak } = dotMatrixOpacitySettings(settings);
    return remapOpacityToTriplet(clamp(value, 0, 1), base, mid, peak);
};
const remapDotMatrixOpacity = (opacity, settings) => {
    ...
    return opacity.map((frame) => {
        ...
        return typeof frame === 'number'
            ...
            : Object.assign({}, frame, { value: remapDotMatrixOpacityValue(frame.value, settings) });
    });
};
const dotMatrixColorForOpacityValue = (value, settings) => {
    const colors = dotMatrixColorSettings(settings);
    if (!colors || !Number.isFinite(value))
        return undefined;
    const opacity = clamp(value, 0, 1);
    const { base, mid, peak } = dotMatrixOpacitySettings(settings);
    const baseCutoff = (base + mid) / 2;
    const peakCutoff = (mid + peak) / 2;
    if (opacity <= baseCutoff)
        return colors.base;
    if (opacity <= peakCutoff)
        return colors.mid;
    return colors.peak;
};
const remapDotMatrixFill = (opacity, settings) => {
    ...
    return opacity.map((frame) => {
        ...
        return Object.assign({}, frame, { value: color });
    });
};
const applyPresetSettings = (animation, settings, isDotMatrixPreset) => {
    ...
    const web = isDotMatrixPreset
        ? Object.assign({}, animation.web, { opacity: remapDotMatrixOpacity(animation.web && animation.web.opacity, settings) }, (dotMatrixFill ? { fill: dotMatrixFill } : {})) : animation.web;
    return Object.assign({}, animation, { from: typeof animation.from === 'number' ? animation.from / speed : animation.from, duration: typeof animation.duration === 'number'
            ? animation.duration / speed
            : animation.duration, web });
};
const wrapPreset = (animation, isDotMatrixPreset) => (targets, x, y, count, entity, settings) => applyPresetSettings(animation(targets, x, y, count, entity, settings), settings, isDotMatrixPreset);
const resolveAnimationPreset = (name) => {
    switch (name) {
...
export const getAnimationPreset = (name) => wrapPreset(resolveAnimationPreset(name), dotMatrixAnimationPresets.indexOf(name) > -1);
