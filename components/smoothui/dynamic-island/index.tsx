...
const DefaultIdle = ({ compact = false }: { compact?: boolean }) => {
...
export interface DynamicIslandProps {
  appearance?: "default" | "desktop-glass";
  className?: string;
...
export default function DynamicIsland({
  ...
  const isDesktopGlass = appearance === "desktop-glass";
  ...
  const content = useMemo(() => {
    switch (view) {
      ...
      case "music":
        return <MusicPlayer />;
      default:
        return idleContent ?? <DefaultIdle compact={isDesktopGlass} />;
    }
  }, [view, idleContent, isDesktopGlass, ringContent, timerContent]);
  ...
  return (
    <div className={showViewControls ? `h-[200px] ${className}` : className}>
      <div
        className={
          showViewControls
